...
async def download_media(url: str, quality: str, updater: ProgressUpdater, user_cookies: str = None):
    ...
    if quality == "720p":
        ytdlp_args = ["-f", "bestvideo[height<=720]+bestaudio[ext=m4a]/bestvideo[height<=720]+bestaudio/best[height<=720]/best", "--merge-output-format", "mp4"]
    elif quality == "480p":
        ytdlp_args = ["-f", "bestvideo[height<=480]+bestaudio[ext=m4a]/bestvideo[height<=480]+bestaudio/best[height<=480]/best", "--merge-output-format", "mp4"]
    elif quality == "360p":
        ytdlp_args = ["-f", "bestvideo[height<=360]+bestaudio[ext=m4a]/bestvideo[height<=360]+bestaudio/best[height<=360]/best", "--merge-output-format", "mp4"]
    elif quality == "audio":
        ytdlp_args = ["-x", "--audio-format", "mp3"]
    else:
        ytdlp_args = ["-f", "bestvideo+bestaudio[ext=m4a]/bestvideo+bestaudio/best", "--merge-output-format", "mp4"]
    ...
    async def run_ytdlp(use_cookies: bool) -> tuple[int, list[str]]:
        ...
        logging.info(f"[yt-dlp CMD]: {' '.join(shlex.quote(c) for c in cmd_parts)}")

        process = await asyncio.create_subprocess_exec(
            *cmd_parts,                # <-- exec نه shell
