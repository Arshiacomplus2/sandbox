...
async def push_to_github(user_id: int, user: User, file_paths: list, upload_method: str, updater: ProgressUpdater):
    if upload_method == "api":
        return await push_to_github_api(user_id, user, file_paths, updater)
    else:
        return await push_to_github_clone(user_id, user, file_paths, updater)

async def push_to_github_clone(user_id: int, user: User, file_paths: list, updater: ProgressUpdater):
...
async def push_to_github_api(user_id: int, user: User, file_paths: list, updater: ProgressUpdater):
    repo = user.github_repo
    token = user.github_token
